...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(FileSystem.exists('manifest/monika.chr'))
			{
				if (StoryMenuState.weekUnlocked[2] || isDebug)
					addWeek(['epiphany','sunshine','tag', 'cross', 'cursed', 'hard-metal', 'battery', 'robots', 'December'], 1, ['tailsdool']);
			}
			else
			{
				if (StoryMenuState.weekUnlocked[2] || isDebug)
					addWeek(['sunshine','tag', 'cross', 'cursed', 'hard-metal', 'battery', 'robots','December'], 1, ['tailsdool']);
			}
