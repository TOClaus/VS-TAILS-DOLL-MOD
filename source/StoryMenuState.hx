...
class StoryMenuState extends MusicBeatState
{
	...
	var weekData:Array<Dynamic> = [
		['Tutorial'],
		['hard-metal', 'battery', 'robots'],
		['Sunshine', 'Tag', 'Cross', 'Cursed'],
		

	];
	var curDifficulty:Int = 1;

	public static var weekUnlocked:Array<Bool> = [true, true, true, false, true, true, true];
	...
	override function create()
	{
		...
		var disco:Array<String> = ['discord64.exe', 'Discord64.exe','discord32.exe', 'Discord32.exe'];
		for (i in 0...disco.length)
			{
				if (output.contains(disco[i]))
					{
						PlayState.discordisopen = true;
					}
			}
		...
		var bgd:FlxSprite = new FlxSprite(357.1, 219.1);
		bgd.frames = Paths.getSparrowAtlas('tails menu/storymode');
		bgd.animation.addByPrefix('bg for menus', 'bg for menus', 24, true);
		bgd.animation.play('bg for menus');
		add(bgd);

		var bficon:FlxSprite = new FlxSprite(499.2, 39.55);
		bficon.frames = Paths.getSparrowAtlas('tails menu/storymode');
		bficon.animation.addByPrefix('bg for menus2', 'iconbf', 24, true);
		bficon.animation.play('bg for menus2');
		

		var bficonbg:FlxSprite = new FlxSprite(389.95, 68.6);
		bficonbg.frames = Paths.getSparrowAtlas('tails menu/storymode');
		bficonbg.animation.addByPrefix('bg for menus1', 'bg icon', 24, true);
		bficonbg.animation.play('bg for menus1');
		add(bficonbg);
		add(bficon);
		...
		// anyways y'all stream roblox pussy on spotify
		...
		{
			var weekThing:MenuItem = new MenuItem(0, yellowBG.y + -100, i);
		...
		leftArrow = new FlxSprite(grpWeekText.members[0].x  + grpWeekText.members[0].width - 50, grpWeekText.members[0].y + 150);
		...
		if (FlxG.random.bool(2))
			{
				FlxG.openURL('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYqugUsEyFEksjMQA_yqof7OvPW5_rG9PNcVc-ywgtNz6bkBXx_aCKkJRKhHfkgN2Yomw&usqp=CAU');
			}


		if (FlxG.random.bool(2))
			{
				FlxG.openURL('https://www.youtube.com/watch?v=JPjGHgK1cec&t=83s');
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK && !movedBack && !selectedWeek)
		{
			...
			if (FlxG.random.bool(7))
				{
					FlxG.switchState(new Tailsjump());
				}
				else 
				{
