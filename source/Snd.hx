...
class Snd {
	...
	//@:noDebug
	public static function loadSound( path:String, streaming : Bool, noteType : Bool  ) : Sound {
		...
		if ( !noteType ) 	mode |= FMOD_NONnoteType;
	...
	public static function load( path:String, streaming=false,noteType=true ) : Snd {
		var s : Sound = loadSound(path, streaming, noteType);
		if ( s == null) {
			#if !prod
	...
	public static function loadSingleBank( filename : String ) : Null<faxe.Faxe.FmodStudioBankRef>{
		...
		var result = fsys.loadBankFile( 
			...
			FmodStudioLoadBank.FMOD_STUDIO_LOAD_BANK_NONnoteType, 
