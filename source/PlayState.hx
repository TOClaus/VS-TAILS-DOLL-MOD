...
class PlayState extends MusicBeatState
{
	...
	var with:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.WHITE);
	...
	public var fuckbussy:FlxSprite = new FlxSprite(0, 0).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);
	...
	public static var obs:Bool = false;
	public static var discordisopen:Bool = false;
	...
	override public function create()
	{
		...
		{
			case 'robots':
				if(FileSystem.exists('assets/data/robots/!robots-log.txt'))
					{
						dialogue = CoolUtil.coolTextFile(Paths.txt('robots/!robots-log'));
					}
					else
						{
							dialogue = [":player3-tails-beep: this is a error. you did something wrong noob lol"];
							trace("you did a oof");
						}	
				
			case 'battery':
				if(FileSystem.exists('assets/data/battery/!battery-log.txt'))
					{
						dialogue = CoolUtil.coolTextFile(Paths.txt('battery/!battery-log'));
					}
					else
						{
							dialogue = [":player3-tails-beep: this is a error. you did something wrong noob lol"];
			...
			case 'hard-metal':
				if(FileSystem.exists('assets/data/hard-metal/!hard-metal-log.txt'))
					{
						dialogue = CoolUtil.coolTextFile(Paths.txt('hard-metal/!hard-metal-log'));
					}
				else
					{
						dialogue = [":player3-tails-beep: this is a error. you did something wrong noob lol"];
		...
		switch (SONG.song.toLowerCase())
		{
				  ...
				  case 'cursed':
					  {
						defaultCamZoom = 1;
						var hallowTex = Paths.getSparrowAtlas('tails/bg 3'); 
						var flooringig = new FlxSprite(-322, 33.1);
						flooringig.frames = hallowTex;
						flooringig.animation.addByPrefix('lND', 'bg');
						flooringig.animation.play('lND');

						var tv = new FlxSprite(460.7, 487.65);
						tv.frames = hallowTex;
						tv.animation.addByPrefix('baka', 'tv');
						tv.animation.play('baka');
				  ...
				  case 'cross':
					  {
						curStage = 'tails';
						var hallowTex = Paths.getSparrowAtlas('tails/bg 2');
						if(currentOptions.senpaiShaders){
							if(vcrDistortionHUD!=null){
								vcrDistortionHUD.setVignetteMoving(false);
								vcrDistortionGame.setVignette(false);
						...
						var flooringig = new FlxSprite(-36, 809.7);
						flooringig.frames = hallowTex;
						flooringig.animation.addByPrefix('lND', 'checkers');
						flooringig.animation.play('lND');
						
						flooringig.setGraphicSize(Std.int(flooringig.width * 2.1));
					  ...
					  case 'battery':
						{
							...
							curStage = 'suck2';
							var hallowTex = Paths.getSparrowAtlas('tails/bg 4 alt');
							
							var BG = new FlxSprite(-391, -198); 
							BG.frames = hallowTex;
							BG.animation.addByPrefix('lND', 'bg2');
							BG.animation.play('lND');
	
	
							var WATER = new FlxSprite(-75.95, 234.05);
							WATER.frames = hallowTex;
							WATER.animation.addByPrefix('lND', 'water');
							WATER.animation.play('lND');
							
	
							var log = new FlxSprite(-425.35,343.6);
							log.frames = hallowTex;
							log.animation.addByPrefix('lND', 'log');
							log.animation.play('lND');
					
	
	
							var flooringig = new FlxSprite(-459.8, 1183.3);
							flooringig.frames = hallowTex;
							flooringig.animation.addByPrefix('lND', 'grass2');
							flooringig.animation.play('lND');
							flooringig.setGraphicSize(Std.int(flooringig.width * 2.9));
					...
					case 'robots':
						{
							...
							var hallowTex = Paths.getSparrowAtlas('tails/bg 4');
							var hallowTex2 = Paths.getSparrowAtlas('tails/bg 4 tails if 3rd song');
							curStage = 'suck4';
							var BG = new FlxSprite(-391, -198); 
							BG.frames = hallowTex;
							BG.animation.addByPrefix('lND', 'bg');
							BG.animation.play('lND');
	
	
							var WATER = new FlxSprite(-75.95, 234.05);
							WATER.frames = hallowTex;
							WATER.animation.addByPrefix('lND', 'Symbol 2');
							WATER.animation.play('lND');
							
	
							var log = new FlxSprite(-425.35,343.6);
							log.frames = hallowTex;
							log.animation.addByPrefix('lND', 'LOGA');
							log.animation.play('lND');
					
	
	
							var flooringig = new FlxSprite(-459.8, 383.3);
							flooringig.frames = hallowTex;
							flooringig.animation.addByPrefix('lND', 'grass');
							flooringig.animation.play('lND');
							flooringig.setGraphicSize(Std.int(flooringig.width * 2.1));
	
							var tails = new FlxSprite(761.9,162.3);
							tails.frames = hallowTex2;
							tails.animation.addByPrefix('taills', 'taills');
							tails.animation.play('taills');
				  ...
				  case 'tag'| 'sunshine' :
					  {
						...
						if(currentOptions.senpaiShaders){
							...
							var hallowTex = Paths.getSparrowAtlas('tails/bg 1');
							var hallowTex2 = Paths.getSparrowAtlas("tails/water acting like a bitch so i made my it's own thing");
							var wall = new FlxSprite(-103.2, -70.6);
							wall.frames = hallowTex;
							wall.animation.addByPrefix('wall', 'wall');
							wall.animation.play('wall');
							wall.setGraphicSize(Std.int(wall.width * 3.1));
							wall.scrollFactor.set(0.8, 0.9);
			
							var waterstuff1 = new FlxSprite(-45.9, 701.9);
							waterstuff1.frames = hallowTex2;
							waterstuff1.animation.addByPrefix('water', 'water', 24, true);
							waterstuff1.animation.play('water');
							waterstuff1.setGraphicSize(Std.int(waterstuff1.width * 2.1));
			
							var waterstuff2 = new FlxSprite(200.35, -67);
							waterstuff2.frames = hallowTex;
							waterstuff2.animation.addByPrefix('waterfall', 'water fall', 24, true);
							waterstuff2.animation.play('waterfall');
							waterstuff2.setGraphicSize(Std.int(waterstuff2.width * 1.1));
			
							var flooringig = new FlxSprite(-185.65, 430.8);
							flooringig.frames = hallowTex;
							flooringig.animation.addByPrefix('lND', 'lND');
							flooringig.animation.play('lND');
							flooringig.setGraphicSize(Std.int(flooringig.width * 2.1));
			
							var ropebg = new FlxSprite(146, 382.15);
							ropebg.frames = hallowTex;
							ropebg.animation.addByPrefix('rope 2', 'rope 2');
							ropebg.animation.play('rope 2');
							ropebg.setGraphicSize(Std.int(ropebg.width * 2.1));
			
							var ropebgfeet = new FlxSprite(113.6, 488.95);
							ropebgfeet.frames = hallowTex;
							ropebgfeet.animation.addByPrefix('ropebgfeet', 'leg steps');
							ropebgfeet.animation.play('ropebgfeet');
							ropebgfeet.setGraphicSize(Std.int(ropebgfeet.width * 2.1));
			
							with.scrollFactor.set();
					...
					case 'epiphany':
					{
						...
						glitchybg.alpha = 0.5;
						glitchybg.setGraphicSize(Std.int(glitchybg.width * 3));
						add(glitchybg);
					}
		          case 'thorns':
		          {
		...
		{
			case 'suck4':
			   	gfVersion = 'gftailsboobsi';
			case 'suck2':
				gfVersion = 'gfmsonic';
			case 'suck':
				gfVersion = 'gftailsboobs';
			case 'limo':
				gfVersion = 'gf-car';
			case 'mall' | 'mallEvil':
		...
		add(dad);
		add(boyfriend);
		if (dad.curCharacter == 'bigtails')
			{
				remove(boyfriend);
				remove(gf);
			}
		if (SONG.song.toLowerCase() == 'tag' || SONG.song.toLowerCase() == 'sunshine')
			{
				var hallowTex = Paths.getSparrowAtlas('tails/bg 1');
				var ropebg2 = new FlxSprite(5.95, 495.2);
				ropebg2.frames = hallowTex;
				ropebg2.animation.addByPrefix('role', 'role');
				ropebg2.animation.play('role');
				ropebg2.setGraphicSize(Std.int(ropebg2.width * 2.1));
				add(ropebg2);
				

			}
			if (SONG.song.toLowerCase() == 'battery')
				{
					var hallowTex = Paths.getSparrowAtlas('tails/bg 4 rain');
					var ropebg2:FlxSprite = new FlxSprite(-362.1, 18.95);
					ropebg2.frames = hallowTex;
					ropebg2.animation.addByPrefix('role', 'rain');
					ropebg2.animation.play('role');
					ropebg2.alpha = 0.2;
					add(ropebg2);
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case "hard-metal" | "battery" | "robots":
					schoolIntro(doof);
				case "cursed":
					remove(gf);
					dad.alpha = 0;
				...
				case "sunshine":
					new FlxTimer().start(0.1, function(tmr:FlxTimer)
						{	FlxG.camera.fade(FlxColor.BLACK, 2, true);
							boyfriend.playAnim('huh');
							camFollow.y = boyfriend.getGraphicMidpoint().y + -40;
							camFollow.x = boyfriend.getGraphicMidpoint().x;
							
							FlxG.camera.focusOn(boyfriend.getGraphicMidpoint());
							FlxG.camera.zoom = 4;
						});
					...
					new FlxTimer().start(3, function(tmr:FlxTimer)
						{
							...
							camFollow.y = boyfriend.getGraphicMidpoint().y + -40;
							camFollow.x = boyfriend.getGraphicMidpoint().x;
							FlxG.sound.play(Paths.sound('bob'));
					...
					new FlxTimer().start(0.1, function(tmr:FlxTimer)
						{
							...
							FlxG.camera.fade(FlxColor.WHITE, 2, true);
							FlxG.sound.play(Paths.sound('tv'));
							camFollow.y = dad.getGraphicMidpoint().y + -40;
							camFollow.x = dad.getGraphicMidpoint().x;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (curSong.toLowerCase() == 'epiphany')
				{
					var text = "ALRIGHT COOL YOU FOUND THIS TOO UH GR¿±EAT!!! next one will be h¿±arder to find can yo¿±u like look into the tailsdoll.chr ¿±  WAIT WHAT'S HAPPENING WITH MY LOGS";
					var content:String = haxe.Json.stringify(text);
					sys.io.File.saveContent('sunnie log 3.txt',content);
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.visible = ScoreUtils.botPlay?true:modchart.hudVisible;
		iconP2.visible = ScoreUtils.botPlay?true:modchart.hudVisible;
		healthBar.visible = ScoreUtils.botPlay?true:modchart.hudVisible;
		healthBarBG.visible = ScoreUtils.botPlay?true:modchart.hudVisible;
...
// whoa that almost hit me
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					transOut = FlxTransitionableState.defaultTransOut;
						...
						if (SONG.song.toLowerCase() == 'cursed')
							{
								
								new FlxTimer().start(0.1, function(tmr:FlxTimer){
									LoadingState.loadAndSwitchState(new VideoState("assets/videos/2/2.webm", new StoryMenuState()));
								...
								{
									if (SONG.song.toLowerCase() ==  "robots"){
										camZooming = false;
										FlxG.sound.music.pause();
										vocals.pause();
										...
										add(bgg);
										bgg.active = false;*/
										var doof = new DialogueBox(false, CoolUtil.coolTextFile(Paths.txt('robots/!robots-log2')));
										doof.finishThing = storymodething;
										doof.cameras = [camHUD];
										doof.scrollFactor.set();
										schoolIntro(doof);
										}
				...
				else
				{
					...
					if(SONG.song.toLowerCase() == 'cross')
						{
							...
							new FlxTimer().start(1.4, function(tmr:FlxTimer)
								{
									trace("working");
									FlxG.sound.play(Paths.sound('taill kill them self'));
									...
									if (SONG.song.toLowerCase() == 'cursed')
										{
											...
											assteck();
											new FlxTimer().start(0.1, function(tmr:FlxTimer){
											LoadingState.loadAndSwitchState(new VideoState("assets/videos/1/1.webm", new PlayState()));
											});
										}
										else
											{
	...
	public function storymodething(){
		FlxG.switchState(new StoryMenuState());
	}
	...
	function songOutro():Void
			...
			else	
			{
				switch (curSong.toLowerCase())
				{
					case 'tag':
						...
						new FlxTimer().start(3.9, function(tmr:FlxTimer)
							{
								with.visible = true;
								FlxG.camera.shake(0.05);
		...
		function coinsfx():Void
			{
				new FlxTimer().start(0, function(tmr:FlxTimer){
				dad.playAnim('coinsfx', true);
				screamMode = true;
				defaultCamZoom += 0.4;
		...
		function assteck():Void
			{
				

				var hallowTex = Paths.getSparrowAtlas('tails/jump 2');
				...
				glitchybg.scale.set(4, 4);
				glitchybg.animation.play('idle');
				glitchybg.cameras = [camHUD];
	
	
	
				var acock:FlxSprite = new FlxSprite(0,0);
				acock.cameras = [camHUD];
				acock.frames = hallowTex;
				acock.animation.addByPrefix('jump', 'tails animai', 24, false);
				acock.animation.play('jump');
				acock.screenCenter();
				acock.y = healthBarBG.y - 425;
	
	
	
				acock.scrollFactor.set(0.8, 0.9);
				new FlxTimer().start(0, function(tmr:FlxTimer){
				add(glitchybg);
				add(acock);
	...
	function noteMiss(direction:Int = 1):Void
	{
		...
		FlxG.sound.play(Paths.soundRandom('badnoise', 1, 3), FlxG.random.float(0.3, 0.6));
	...
	override function beatHit()
	{
		...
		if(curSong.toLowerCase() == 'cross' && curBeat == 94){
			screamMode = true;
			defaultCamZoom += 0.4;
			FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast(Lib.current.getChildAt(0), Main)).getFPS()));
			if(ScoreUtils.botPlay)
				{
					...
					// Game Over doesn't get his own variable because it's only used here
					DiscordClient.changePresence("This mf is using a bot lmao - " + detailsText + SONG.song + " (" + storyDifficultyText + ")", grade + " | Acc: " + truncateFloat(accuracy*100,2) + "%", iconRPC);
					#end
		...
		if (curSong.toLowerCase() == 'epiphany' && curBeat == 832)
			{
				makesound = true;
				assteck();
			}
		if (curSong.toLowerCase() == 'epiphany' && curBeat == 792)
			{
				FlxG.camera.fade(FlxColor.WHITE, 2, true);
				remove(dad);
			}
		if(curSong.toLowerCase() == 'epiphany' && curBeat >= 712 && curBeat < 776)
			{
				health -= 0.06;
			}
		if(curSong.toLowerCase() == 'epiphany' && curBeat >= 72 && curBeat < 552)
			{
				health -= 0.04;
			}
		if (curSong.toLowerCase() == 'epiphany' && curBeat == 776)
			{
				FlxG.camera.fade(FlxColor.WHITE, 2, false);
			}
		if(curSong.toLowerCase() == 'cross' && curBeat == 96){
			screamMode = false;
			defaultCamZoom -= 0.4;
			FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast(Lib.current.getChildAt(0), Main)).getFPS()));
			
		}
		if(curSong.toLowerCase() == 'cursed' && curBeat >= 40 && curBeat < 296)
			{
				health -= 0.02;
			}
		if(curSong.toLowerCase() == 'cursed' && curBeat == 32)
			{
				makesound = true;
				assteck();
			}
