...
class HealthIcon extends FlxSprite
{
	...
	public function changeCharacter(char:String){
		if(char=='tailsdool' || char == 'tailsbow' || char == 'tailsbow-sad' || char == 'bigtails')
			{
				loadGraphic(Paths.image('icon-tailsdool'), true, 150, 150);
				animation.add('tailsdool', [0, 1], 0, false);
				animation.add('bigtails', [0, 1], 0, false);
				animation.add('tailsbow', [0, 1], 0, false);
				animation.add('tailsbow-sad', [0, 1], 0, false);
				
				
			}
		else
			{
				if(char== 'msonic' ||char== 'mkuk'  )
					{
						loadGraphic(Paths.image('icon-son'), true, 150, 150);
						animation.add('msonic', [0, 1], 0, false);
						animation.add('mkuk', [0, 1], 0, false);
					}
					else
						{
		...
		animation.add('bf-car2', [0, 1], 0, false);
